...
public class ContasPesquisar extends Janela {

    private final String Acao;
    private final Contas conta;

    public ContasPesquisar(Contas Conta, String acao) {
        initComponents();
    ...
    public void Filtrar()
    {
        try
        {
            Banco.executeQuery("SELECT * FROM contas WHERE nome LIKE '%"+nome.getText()+"%' ORDER BY nome ASC");
            DefaultTableModel modelo = (DefaultTableModel)resultado_pesquisa.getModel();
            modelo.setNumRows(0);
            String ativar,saldototal;
            while(Banco.getResultSet().next())
            {
                if(Banco.getResultSet().getInt("ativada") == 0)
                    ativar = Lingua.getMensagem("sim");
                else
                    ativar = Lingua.getMensagem("nao");
                if(Banco.getResultSet().getInt("saldo_total") == 0)
                    saldototal = Lingua.getMensagem("sim");
                else
                    saldototal = Lingua.getMensagem("nao");
                modelo.addRow(new Object[]{Banco.getResultSet().getString("id_conta"),Banco.getResultSet().getString("nome"),Banco.getResultSet().getString("valor"),ativar,saldototal});
            }
    ...
    private void initComponents() {
        ...
        jPanel1.setBorder(javax.swing.BorderFactory.createTitledBorder(null, Lingua.getMensagem("contas"), javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, FonteFinancas, new java.awt.Color(0, 102, 204)));
        ...
        if (resultado_pesquisa.getColumnModel().getColumnCount() > 0) {
            ...
            resultado_pesquisa.getColumnModel().getColumn(2).setHeaderValue(Lingua.getMensagem("saldo"));
            resultado_pesquisa.getColumnModel().getColumn(3).setHeaderValue(Lingua.getMensagem("ativada"));
            resultado_pesquisa.getColumnModel().getColumn(4).setHeaderValue(Lingua.getMensagem("saldo_total"));
        }
...
private void resultado_pesquisaMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_resultado_pesquisaMouseClicked
    ...
    if(Janelinha.Pergunta(codSelecionado1, Lingua.getMensagem("tem_certeza"))){
        conta.PreencherFormulario(Acao,codSelecionado0,codSelecionado1,codSelecionado2,codSelecionado3,codSelecionado4);
        ContasPesquisar.this.dispose();
    }
