...
public final class DespesasItem extends Janela {
    ...
    public void PreencherFormulario(String acao, String item_id, String Item, String nome_categoria)
    {
        ...
        if(Acao==null)
        {
            ...
            if(Acao.equals(Lingua.getMensagem("cadastrar")))
            {
                ...
                try
                {
                    Banco.executeQuery("SELECT nome FROM despesas_itens WHERE id_item='"+id_item+"'");
                    Banco.getResultSet().next();
                    item.setText(Banco.getResultSet().getString("nome"));
                }catch(Exception e){
                  item.setText(Lingua.getMensagem("erro")+": "+e.getMessage());
                }
    ...
    public void Cadastrar()
    {
        String id_categoria="";
        try{
            Banco.executeQuery("SELECT id_categoria FROM despesas_categorias WHERE nome='"+categoria.getSelectedItem()+"'");
            Banco.getResultSet().next();
        ...
        try{
            Banco.executeUpdate("INSERT INTO despesas_itens (id_categoria,nome) VALUES('"+id_categoria+"','"+item.getText()+"')");
    ...
    public void Alterar()
    {
        ...
        try{
            Banco.executeUpdate("UPDATE despesas_itens SET id_categoria='"+id_categoria+"', nome='"+item.getText()+"' WHERE id_item='"+id_item+"'");
    ...
    {
        try{
            Banco.executeQuery("SELECT * FROM despesas WHERE id_item='"+id_item+"'");
            if(Banco.getResultSet().next())
            {
                Janelinha.Aviso(Lingua.getMensagem("erro"),Lingua.getMensagem("restricao_excluir"));
                PreencherFormulario(null,null,null,null);
            }else{
                Banco.executeUpdate("DELETE FROM despesas_itens WHERE id_item='"+id_item+"'");
    ...
    public void Botoes(String acao)
    {
        if(acao.equals(Lingua.getMensagem("cadastrar"))){
            ...
            new DespesasItemPesquisar(this,Lingua.getMensagem("alterar")).setVisible(true);
        }else if(acao.equals(Lingua.getMensagem("excluir"))){
            new DespesasItemPesquisar(this,Lingua.getMensagem("excluir")).setVisible(true);
        }else if(acao.equals(Lingua.getMensagem("consultar"))){
            new DespesasItemPesquisar(this,Lingua.getMensagem("consultar")).setVisible(true);
        }else if(acao.equals(Lingua.getMensagem("sair"))){
            DespesasItem.this.dispose();
        }
    }
    ...
    private void initComponents() {
        ...
        setTitle(Lingua.getMensagem("despesas")+" > "+Lingua.getMensagem("itens"));
        ...
        PainelItem.setBorder(javax.swing.BorderFactory.createTitledBorder(null, Lingua.getMensagem("dados_item"), javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, FonteFinancas, new java.awt.Color(0, 102, 204)));
        PainelItem.setForeground(new java.awt.Color(51, 94, 168));
        ...
        PainelItem.add(CIDADE);
        CIDADE.setBounds(17, 70, 80, 20);

        categoria.setFont(FonteFinancas);
        PainelItem.add(categoria);
        categoria.setBounds(110, 70, 243, 20);
        ...
        PainelItem.add(BFinalizar);
        BFinalizar.setBounds(110, 100, 120, 20);
        ...
        PainelItem.add(item);
        item.setBounds(110, 40, 243, 20);

        getContentPane().add(PainelItem);
        PainelItem.setBounds(20, 60, 387, 145);
