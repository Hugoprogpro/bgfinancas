...
public final class TransferenciasCategoria extends Janela {

    private String Acao;
    private String id_categoria;

    public TransferenciasCategoria() {
        initComponents();
    ...
    {    
        try{
            Banco.executeUpdate("INSERT INTO transferencias_categorias (nome) VALUES('"+nome.getText()+"')");
    ...
    {     
        try{
            Banco.executeUpdate("UPDATE transferencias_categorias SET nome='"+nome.getText()+"' WHERE id_categoria='"+id_categoria+"'");
    ...
    {    
        try{
            Banco.executeQuery("SELECT * FROM transferencias_itens WHERE id_categoria='"+id_categoria+"'");
            if(Banco.getResultSet().next())
            {
                Janelinha.Aviso(Lingua.getMensagem("erro"),Lingua.getMensagem("restricao_excluir"));
                PreencherFormulario(null,null,null);
            }else{
                if(Banco.executeUpdate("DELETE FROM transferencias_categorias WHERE id_categoria='"+id_categoria+"'")>0)
                {
    ...
    public void Botoes(String acao)
    {
        if(acao.equals(Lingua.getMensagem("cadastrar"))){
            ...
            new TransferenciasCategoriaPesquisar(this,Lingua.getMensagem("alterar")).setVisible(true);
        }else if(acao.equals(Lingua.getMensagem("excluir"))){
            new TransferenciasCategoriaPesquisar(this,Lingua.getMensagem("excluir")).setVisible(true);
        }else if(acao.equals(Lingua.getMensagem("consultar"))){
            new TransferenciasCategoriaPesquisar(this,Lingua.getMensagem("consultar")).setVisible(true);
        }else if(acao.equals(Lingua.getMensagem("sair"))){
            TransferenciasCategoria.this.dispose();
        }
    }
    ...
    private void initComponents() {
        ...
        setTitle(Lingua.getMensagem("transferencias")+" > "+Lingua.getMensagem("categoria"));
        ...
        BarraDeFerramentas.setBounds(0, 0, 480, 41);
        ...
        BFinalizar.setBounds(100, 70, 130, 20);

        getContentPane().add(PainelCategoria);
        PainelCategoria.setBounds(20, 60, 374, 118);

        setSize(new java.awt.Dimension(414, 219));
