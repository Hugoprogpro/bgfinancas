...
public final class ReceitasCategoria extends Janela {

    private String Acao;
    private String id_categoria;

    public ReceitasCategoria() {
        initComponents();
    ...
    {    
        try{
            Banco.executeUpdate("INSERT INTO receitas_categorias (nome) VALUES('"+nome.getText()+"')");
    ...
    {     
        try{
            Banco.executeUpdate("UPDATE receitas_categorias SET nome='"+nome.getText()+"' WHERE id_categoria='"+id_categoria+"'");
    ...
    {    
        try{
            Banco.executeQuery("SELECT * FROM receitas_itens WHERE id_categoria='"+id_categoria+"'");
            if(Banco.getResultSet().next())
            {
                Janelinha.Aviso(Lingua.getMensagem("erro"),Lingua.getMensagem("restricao_excluir"));
                PreencherFormulario(null,null,null);
            }else{
                if(Banco.executeUpdate("DELETE FROM receitas_categorias WHERE id_categoria='"+id_categoria+"'")>0)
                {
    ...
    public void Botoes(String acao)
    {
        if(acao.equals(Lingua.getMensagem("cadastrar"))){
            ...
            new ReceitasCategoriaPesquisar(this,Lingua.getMensagem("alterar")).setVisible(true);
        }else if(acao.equals(Lingua.getMensagem("excluir"))){
            new ReceitasCategoriaPesquisar(this,Lingua.getMensagem("excluir")).setVisible(true);
        }else if(acao.equals(Lingua.getMensagem("consultar"))){
            new ReceitasCategoriaPesquisar(this,Lingua.getMensagem("consultar")).setVisible(true);
        }else if(acao.equals(Lingua.getMensagem("sair"))){
            ReceitasCategoria.this.dispose();
        }
    }
    ...
    private void initComponents() {
        ...
        setTitle(Lingua.getMensagem("receitas")+" > "+Lingua.getMensagem("categoria"));
        ...
        NOME.setBounds(17, 40, 70, 20);

        nome.setFont(FonteFinancas);
        PainelCategoria.add(nome);
        nome.setBounds(100, 40, 240, 20);
        ...
        PainelCategoria.setBounds(20, 60, 370, 115);

        setSize(new java.awt.Dimension(419, 217));
