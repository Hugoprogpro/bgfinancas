...
public class AgendaTipoPesquisar extends Janela {

    private final String Acao;
    private final Agenda agenda;
    private final AgendaTipo agenda_categoria;
    
    public AgendaTipoPesquisar(AgendaTipo _agenda_categoria, Agenda _agenda, String acao) {
        initComponents();
        Visual.Janela(this.getJanela());
        BarraDeFerramentas.add(Botoes.Sair(this));
        Acao = acao;
        agenda_categoria = _agenda_categoria;
        agenda = _agenda;
    ...
    public void Filtrar()
    {
        try
        {
            Banco.executeQuery("SELECT * FROM agenda_tipos WHERE nome LIKE '%"+nome.getText()+"%' ORDER BY nome ASC");
            DefaultTableModel modelo = (DefaultTableModel)resultado_pesquisa.getModel();
            modelo.setNumRows(0);
            while(Banco.getResultSet().next())
            {
                modelo.addRow(new Object[]{Banco.getResultSet().getString("id_tipo"),Banco.getResultSet().getString("nome")});
            }
    ...
    public void Botoes(String acao)
    {
        if(acao.equals(Lingua.getMensagem("sair"))){
            AgendaTipoPesquisar.this.dispose();
        }
    }
    ...
    private void initComponents() {
        ...
        jPanel1.setBorder(javax.swing.BorderFactory.createTitledBorder(null, Lingua.getMensagem("tipo_lembrete"), javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, FonteFinancas, new java.awt.Color(0, 102, 204)));
...
private void resultado_pesquisaMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_resultado_pesquisaMouseClicked
    ...
    if(Janelinha.Pergunta(codSelecionado1, Lingua.getMensagem("tem_certeza"))){
        if(agenda_categoria!=null)
        {
            agenda_categoria.PreencherFormulario(Acao,codSelecionado0,codSelecionado1);
            AgendaTipoPesquisar.this.dispose();
        }else{
            agenda.AdicionarItem(codSelecionado1);
            AgendaTipoPesquisar.this.dispose();
        }
    }
